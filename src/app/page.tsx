    ...
    <div className="flex h-screen w-screen bg-zinc-100 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50 overflow-hidden">
      ...
      <aside className="w-64 border-r border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900/50 flex flex-col gap-6 select-none">
        ...
        <nav className="space-y-1">
          ...
          <button
            type="button"
            onClick={() => { setSelectedTag(null); setDateFilter("all"); setStartDate(""); setEndDate(""); setSearchQuery(""); }}
          ...
          <div className="pt-2 mt-2 border-t border-zinc-100 dark:border-zinc-800/50 space-y-1">
            ...
            <button
              type="button"
              onClick={() => { setDateFilter("today"); setStartDate(""); setEndDate(""); }}
            ...
            <button
              type="button"
              onClick={() => { setDateFilter("week"); setStartDate(""); setEndDate(""); }}
            ...
            <button
              type="button"
              onClick={() => { setDateFilter("month"); setStartDate(""); setEndDate(""); }}
            ...
            <div className="pt-2 mt-2 border-t border-zinc-100 dark:border-zinc-800/40 space-y-1.5">
              <p className="px-2 text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Rango personalizado</p>
              <div className="grid grid-cols-2 gap-1.5 px-2">
                <div>
                  <label className="text-[9px] text-zinc-400 block mb-0.5">Desde</label>
                  <input
                    type="date"
                    value={startDate}
                ...
                <div>
                  <label className="text-[9px] text-zinc-400 block mb-0.5">Hasta</label>
                  <input
                    type="date"
                    value={endDate}
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-zinc-950">
        ...
        <section className="flex-1 overflow-y-auto p-6 bg-zinc-50/50 dark:bg-zinc-950">
          {loading ? (
            <div className="flex h-32 items-center justify-center text-sm text-zinc-500">Cargando tu baúl...</div>
          ) : filteredNotes.length === 0 ? (
            <div className="flex h-32 items-center justify-center text-sm text-zinc-400 italic">No se encontraron registros</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredNotes.map((note) => (
                <div key={note.id} className="flex flex-col rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 justify-between min-h-[220px]">
                  <div>
                    ...
                    <h3 className="text-base font-bold text-zinc-900 dark:text-zinc-50 line-clamp-1">{note.title}</h3>
                    {note.summary && (
                  ...
                  <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800/60 flex items-center justify-between text-[10px] text-zinc-400">
                    ...
                    {note.source_url && (
                      <a href={note.source_url} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline inline-flex items-center gap-0.5 font-medium">
                        Ver enlace 🔗
                      </a>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/40 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="w-full max-w-3xl rounded-xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900 my-8">
            ...
            <div className="mb-4 flex items-center justify-between border-b border-zinc-100 pb-3 dark:border-zinc-800">
              <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Guardar Historial de IA</h2>
              <button onClick={closeModal} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 text-sm">✕</button>
            </div>

            {/* SECTOR DE PESTAÑAS */}
            <div className="mb-6 flex border-b border-zinc-200 dark:border-zinc-700">
              <button type="button" onClick={() => setSourceType("text")} className={sourceType === "text" ? "py-2 px-4 text-sm font-medium border-b-2 border-blue-600 text-blue-600 dark:border-blue-400 dark:text-blue-400" : "py-2 px-4 text-sm font-medium border-b-2 border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"}>
                ✍️ Texto Copiado
              </button>
              <button type="button" onClick={() => setSourceType("url")} className={sourceType === "url" ? "py-2 px-4 text-sm font-medium border-b-2 border-blue-600 text-blue-600 dark:border-blue-400 dark:text-blue-400" : "py-2 px-4 text-sm font-medium border-b-2 border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"}>
                🔗 Enlace del Chat
              </button>
              <button type="button" onClick={() => setSourceType("file")} className={sourceType === "file" ? "py-2 px-4 text-sm font-medium border-b-2 border-blue-600 text-blue-600 dark:border-blue-400 dark:text-blue-400" : "py-2 px-4 text-sm font-medium border-b-2 border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"}>
                📄 Subir Archivo
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={aiModel} onChange={(e) => setAiModel(e.target.value)} className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 focus:outline-none">
              ...
              {sourceType === "text" && (
                <>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-700 dark:text-zinc-300">Título de la nota</label>
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ej: Fix del bug de login en NextJS" className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 focus:outline-none" />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-700 dark:text-zinc-300">Contenido del Chat</label>
                    <textarea rows={6} value={content} onChange={(e) => setContent(e.target.value)} placeholder="Usuario: ¿Cómo arreglo este error?..." className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 focus:outline-none resize-none" />
                  </div>
                </>
              )}
              ...
              {sourceType === "url" && (
                <>
                  <div className="rounded-lg bg-blue-50 p-3 text-xs text-blue-700 dark:bg-blue-950/40 dark:text-blue-300">
                    💡 <strong>Opcional:</strong> Si dejas el título vacío, nuestro servidor extraerá de forma limpia el nombre del chat externo en segundo plano.
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-700 dark:text-zinc-300">Título personalizado (Opcional)</label>
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ej: Mi chat de DeepSeek sobre layouts" className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 focus:outline-none" />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-700 dark:text-zinc-300">URL pública del chat compartido</label>
                    <input type="url" value={sourceUrl} onChange={(e) => setSourceUrl(e.target.value)} placeholder="https://chat.deepseek.com/a/chat/..." className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 focus:outline-none" />
                  </div>
                </>
              )}
              ...
              {sourceType === "file" && (
                <div className="flex flex-col items-center justify-center border-2 border-dashed border-zinc-300 rounded-xl p-8 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800/30">
                  ...
                  <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Módulo de lectura en desarrollo</p>
                  <p className="text-xs text-zinc-400 mt-1">Soporte para PDFs y documentos en el próximo asalto.</p>
                </div>
              )}

              <div className="pt-2">
                <label className="mb-1 block text-xs font-medium text-zinc-700 dark:text-zinc-300">Resumen extendido del hilo</label>
                <textarea 
                  rows={3} 
                  value={summary} 
            ...
            <div className="flex justify-end gap-3 mt-6 pt-3 border-t border-zinc-100 dark:border-zinc-800">
              <button type="button" onClick={closeModal} className="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-700 dark:border-zinc-700 dark:text-zinc-300">
                Cancelar
              </button>
              ...
              <button 
                ...
                className={sourceType === "file" ? "inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium text-white bg-zinc-400 dark:bg-zinc-700 cursor-not-allowed" : "inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 transition-colors"}
              >
                {saving ? "Guardando..." : sourceType === "file" ? "Próximamente... 🛠️" : "Guardar Conversación 🚀"}
              </button>
            </div>
